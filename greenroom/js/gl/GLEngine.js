...
import {
  FEATHER_SCALE, computeLightMatch,
} from '../keying.js';
...
uniform int uMode; // 0=前景(预乘rgb+覆盖度) 1=背景纹理rgb 2=纯色背景rgb
void main() {
  if (uMode == 0) {
...
export class GLEngine {
  ...
  render(video, params, layout) {
    ...
    // 7) 周期性统计：前背景亮度/色彩匹配
    if (this.frameNo % 30 === 0) this._updateStats(cur, bgTex, params);
  ...
  _updateStats(fg, bgTex, params) {
    const gl = this.gl;
    const STAT_N = 16 * 16;
    const p = this.progStat;
    // 背景统计源与合成 Pass 同源：solid 直接取 uBgColor；image/video/blur 取背景纹理
    const solid = params.background.mode === 'solid';
    const bgSourceTex = bgTex || this._bgSource(params) || this.scene.tex;
    const read = (mode) => {
      ...
      p.set1('uFlipY', 0);
      p.set1i('uMode', mode);
      p.set3v('uBgColor', params.background.color);
      gl.activeTexture(gl.TEXTURE0);
      gl.bindTexture(gl.TEXTURE2D, fg.tex);
      p.set1i('uFg', 0);
      gl.activeTexture(gl.TEXTURE1);
      gl.bindTexture(gl.TEXTURE_2D, bgSourceTex);
      p.set1i('uBg', 1);
      gl.drawArrays(gl.TRIANGLES, 0, 3);
      this.passCount++;
    ...
    // 前景：Σ(预乘 rgb 字节) + Σ(alpha 字节)；背景：Σ rgb 字节。
    // 全部保持 0–255 字节口径，computeLightMatch 内部统一把条件均值 ×255，
    // 禁止把前景先除成 0–1（旧实现混口径会把曝光夹到上限、色调比失真）。
    const acc = { rS: 0, gS: 0, bS: 0, aS: 0 };
    const fgBuf = read(0);
    for (let i = 0; i < fgBuf.length; i += 4) {
      acc.rS += fgBuf[i]; acc.gS += fgBuf[i + 1];
      acc.bS += fgBuf[i + 2]; acc.aS += fgBuf[i + 3];
    }
    const bg = { r: 0, g: 0, b: 0 };
    const bgBuf = read(solid ? 2 : 1);
    for (let i = 0; i < bgBuf.length; i += 4) {
      bg.r += bgBuf[i]; bg.g += bgBuf[i + 1]; bg.b += bgBuf[i + 2];
    }
    bg.r /= STAT_N; bg.g /= STAT_N; bg.b /= STAT_N;
    const prev = { exp: this.matchExp, tint: this.matchTint };
    const next = computeLightMatch(acc, bg, STAT_N, prev);
    this.matchExp = next.exp;
    this.matchTint = next.tint;
  }
