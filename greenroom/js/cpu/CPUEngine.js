...
import {
  ...
  clamp01, FEATHER_SCALE, computeLightMatch,
} from '../keying.js';
...
export class CPUEngine {
  ...
  render(video, params, layout) {
    ...
    // 光照/色彩匹配统计累加器：与 GLEngine 16×16 回读完全相同的字节口径——
    //   前景 Σ round(调色后rgb·alpha * 255)（预乘 RGB 字节）+ Σ round(alpha*255)；
    //   背景取背景纹理（GL STAT mode=1：blur 用模糊场景，image/video 用背景素材，
    //   无素材时回退原始场景/solid 色）。
    // computeLightMatch 内部统一把条件前景均值 ×255，禁止再出现 fg 0–1 / bg 0–255 的混算。
    const statAcc = { rS: 0, gS: 0, bS: 0, aS: 0 };
    const statBg = { r: 0, g: 0, b: 0 };
    const hasBgTex = bgData && (params.background.mode === 'blur' ||
      params.background.mode === 'image' || params.background.mode === 'video');
    const statBgSrc = hasBgTex ? bgData
      : params.background.mode === 'solid' ? null : d;
    for (let p = 0, i = 0; i < od.length; i += 4, p++) {
      ...
      // 统计：与 GL STAT_FRAG（rgb*a）+ UNSIGNED_BYTE 回读逐字节等价
      statAcc.rS += Math.round(clamp01(fg[0]) * clamp01(aa) * 255);
      statAcc.gS += Math.round(clamp01(fg[1]) * clamp01(aa) * 255);
      statAcc.bS += Math.round(clamp01(fg[2]) * clamp01(aa) * 255);
      statAcc.aS += Math.round(clamp01(aa) * 255);
      if (statBgSrc) {
        statBg.r += statBgSrc[i]; statBg.g += statBgSrc[i + 1]; statBg.b += statBgSrc[i + 2];
      } else {
        statBg.r += bgc[0] * 255; statBg.g += bgc[1] * 255; statBg.b += bgc[2] * 255;
      }
    ...
    // 统计（与 GLEngine 共用 keying.js::computeLightMatch，同为 0–255 字节口径）
    if (this.frameNo % 30 === 0) {
      const n = w * h;
      statBg.r /= n; statBg.g /= n; statBg.b /= n;
      const next = computeLightMatch(statAcc, statBg, n,
        { exp: this.matchExp, tint: this.matchTint });
      this.matchExp = next.exp; this.matchTint = next.tint;
    }
