...
test('CPU 光照/色彩统计：30 帧后按 0..255 口径更新系数，且未被夹到饱和端', () => {
  const engine = new CPUEngine(new FakeCanvas(320, 180));
  const params = createDefaultParams();
  params.grade.lightMatch = true;
  params.background.colorMatch = true;
  for (let i = 0; i < 30; i++) engine.render(fakeVideo, params, layout);
  // 桩画面：前景（被抠出的暗部）与背景（同一场景）亮度差有限。
  // 旧 0..1 口径下曝光/tint 会被夹到 0.5/0.6 饱和端；正确 0..255 口径应落在区间内部。
  assert.ok(engine.matchExp > 0.5 && engine.matchExp < 2.0,
    `matchExp=${engine.matchExp} 被夹到饱和端，统计量纲仍是错的`);
  for (const t of engine.matchTint) {
    assert.ok(t > 0.6 && t < 1.6, `matchTint 分量 ${t} 被夹饱和`);
  }
});
